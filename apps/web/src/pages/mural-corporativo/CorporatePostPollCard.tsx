...
import { useCorporatePostPollVotes, useVoteCorporatePostPoll } from '../../lib/use-corporate-mural'

/**
 * Enquete do post do Feed. Espelha o `ReviewPollCard` da Resenha — mesma
 * pergunta, mesmo desenho — com um estado que lá não existe: **quem não pode
 * votar**.
 *
 * No Feed o post tem público-alvo, e quem modera enxerga post fora do próprio
 * alcance. Esse viewer vê a enquete e o resultado, mas não vota: o servidor
 * recusa, e mostrar um botão que ele vai recusar seria mentir para o clique.
 * `canVote` vem do DTO justamente para essa distinção.
 */
export function CorporatePostPollCard({
  postId,
  poll,
}: {
  postId: string
  poll: CorporatePostPollDTO
}) {
  const [selectedOptionId, setSelectedOptionId] = useState<string | null>(null)
  const [showVoters, setShowVoters] = useState(false)
  const vote = useVoteCorporatePostPoll()
  // O resultado da própria mutation ganha do prop: assim a barra aparece no
  // clique, sem esperar o refetch do feed.
  const current = vote.data?.post.poll?.id === poll.id ? vote.data.post.poll : poll
  const voters = useCorporatePostPollVotes(postId, current.hasVoted && showVoters)
  ...
  if (current.hasVoted) {
    return (
      <section
        className="mt-sm rounded-xl border border-outline-variant/50 bg-surface-container-high p-md"
        aria-label="Resultado da enquete"
      >
            ...
            return (
              <div key={option.id}>
                <div className="mb-1 flex items-center justify-between gap-sm font-label text-label-sm">
                  <span
                    className={`flex min-w-0 items-center gap-xs ${selected ? 'font-bold text-primary' : 'text-on-surface'}`}
                  >
        ...
        {showVoters && (
          <div
            className="mt-sm rounded-lg border border-outline-variant/40 bg-surface p-sm"
            aria-label="Votos por opção"
          >
            {voters.isLoading ? (
    ...
    >
      ...
      <div className="mt-sm flex flex-col gap-xs" role="radiogroup" aria-label={current.question}>
        {current.options.map((option) => (
          <label
            key={option.id}
            className={`flex items-center gap-sm rounded-lg border px-md py-2 text-body-sm transition-colors ${
              !current.canVote
                ...
                : selectedOptionId === option.id
                  ...
                  : 'cursor-pointer border-outline-variant/60 bg-surface text-on-surface hover:border-primary/60'
            }`}
          >
            <input
              type="radio"
              name={`corporate-poll-${current.id}`}
              value={option.id}
              checked={selectedOptionId === option.id}
              disabled={!current.canVote}
      ...
      <div className="mt-sm flex items-center justify-between gap-sm">
        ...
        {current.canVote ? (
          <button
            ...
            onClick={() => {
              if (selectedOptionId) vote.mutate({ postId, optionId: selectedOptionId })
            }}
          ...
          /* No lugar do botão, o motivo. Sem isso o card ficaria com opções
             apagadas e nenhuma explicação de por quê. */
          <p className="text-body-sm text-on-surface-variant">Esta enquete é de outro público.</p>
        )}
      </div>
    </section>
  )
