                ...
                <li className="flex items-start gap-md">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="mb-sm text-body-sm text-on-surface-variant">
                      Uma linha por pessoa. Setores e squads que ainda não existirem são criados.
                    </p>
                    <button
                      type="button"
  ...
  return (
    <div className="flex flex-col gap-md">
      <p role="status" className="font-label text-label-md text-on-surface">
        {counts.CREATE} a criar · {counts.UPDATE} a atualizar · {counts.UNCHANGED} sem mudança · {counts.SKIP} ignoradas
      ...
      {(plan.sectorsToCreate.length > 0 || plan.squadsToCreate.length > 0) && (
        <div className="rounded-md border border-outline-variant/30 p-md">
          <p className="mb-xs font-label text-label-sm uppercase tracking-wide text-on-surface-variant">
            Além das pessoas, será criado
          </p>
          {plan.sectorsToCreate.length > 0 && (
            <p className="text-body-sm text-on-surface">
              Setores: {plan.sectorsToCreate.map((sector) => sector.name).join(', ')} — nascem com o mínimo
              (Escritório); ajuste as funcionalidades em Administração › Setores.
            </p>
          )}
          {plan.squadsToCreate.length > 0 && (
            <p className="text-body-sm text-on-surface">
              Squads: {plan.squadsToCreate.map((squad) => `${squad.name} (${squad.sectorName})`).join(', ')}
            </p>
          )}
        </div>
      )}
...
function ResultadoPanel({
  ...
  return (
    <div className="flex flex-col gap-md">
      ...
      {(result.sectorsCreated.length > 0 || result.squadsCreated.length > 0) && (
